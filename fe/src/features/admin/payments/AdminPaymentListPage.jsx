...
import AdminPaymentListSkeleton from "@/features/admin/payments/AdminPaymentListSkeleton";
...
import { useAdminPayments } from "@/features/admin/payments/useAdminPayments";
import {
  approveRefundViaApi,
...
function AdminPaymentListPage() {
  ...
  function scrollToPaymentsTable() {
  ...
  const paymentPage = useAdminPagination(filteredPayments, ADMIN_PAGE_SIZES.payments, [
    statusFilter,
    search,
    payments,
  ]);
  ...
  function handleConfirmPayment(paymentId) {
    ...
    confirmPayOsPaymentViaApi(paymentId, user?.username ?? "admin_sehub")
      .then((result) => {
  ...
  function handleQuickCompleteRefund(paymentId) {
    ...
    completeRefundViaApi(paymentId, "Admin xác nhận đã chuyển khoản hoàn tiền", user?.username ?? "admin_sehub")
      .then((result) => {
        showToast(result.message);
        if (result.ok) refresh();
      })
  ...
  function handleQuickApproveRefund(paymentId) {
    ...
    approveRefundViaApi(paymentId, "Admin duyệt yêu cầu hoàn tiền của sinh viên", user?.username ?? "admin_sehub")
      .then((result) => {
        showToast(result.message);
        if (result.ok) refresh();
      })
  ...
  if (loading) {
    return <AdminPaymentListSkeleton />;
  }
  ...
  return (
    ...
    >
      <div className={payStyles.flowBanner}>
        ...
        <div className={payStyles.tokenRules}>
          <div className={payStyles.tokenRuleCard}>
            ...
            <span>
              Basic: {FREE_DAILY_TOKEN_QUOTA} token · Premium: {PREMIUM_DAILY_TOKEN_QUOTA} token (reset 00:00)
            </span>
          </div>
          <div className={payStyles.tokenRuleCard}>
            <strong>Tổng token thưởng (cộng thủ công)</strong>
            <span>Tối đa {MAX_BONUS_TOKEN_BALANCE} token / user — đã đủ 1.000 thì không cộng thêm</span>
          </div>
          <div className={payStyles.tokenRuleCard}>
      ...
      {loadError ? <div className={payStyles.refundHintBanner}>{loadError}</div> : null}

      {pendingRefundRequests.length > 0 ? (
        <div className={payStyles.refundHintBanner}>
          <span>
            Có <strong>{pendingRefundRequests.length}</strong> yêu cầu hoàn tiền từ sinh viên đang chờ duyệt.
          </span>
          <button
            type="button"
            className={payStyles.refundHintAction}
            onClick={() => applyStatusFilter("refund_requested")}
          >
      ...
      <RefundedPaymentsPanel
        ...
        onViewInTable={() => applyStatusFilter("refunded")}
        onStartRefund={() => applyStatusFilter("refund_requested")}
      />
        ...
        <div className={styles.panelHeader}>
          <div>
            ...
            <p className={styles.panelDesc}>
              ...
              <strong>Xác nhận &amp; kích hoạt</strong>. Đơn「Đã kích hoạt」→ cột Thao tác → <strong>Hoàn tiền</strong>.
            </p>
          </div>
        </div>
          ...
          <div className={payStyles.refundHintBanner}>
            ...
            {stats.activated > 0 ? (
              <button
                type="button"
                className={payStyles.refundHintAction}
                onClick={() => applyStatusFilter("activated")}
              >
                  ...
                  return (
                    <tr key={payment.id}>
                      <td className={styles.cellMain}>
                        ...
                        {payment.note ? <span className={payStyles.payId}>{payment.note}</span> : null}
                      </td>
                      <td>@{payment.username}</td>
                      <td>
                        {payment.planLabel}
                        {payment.voucher ? <span className={payStyles.payId}>{payment.voucher}</span> : null}
                      </td>
                      ...
                      <td>
                        <StatusBadge status={meta?.status ?? "draft"} label={meta?.label ?? payment.status} />
                      </td>
                      <td>
      ...
      <AdminPaymentAuditSection auditLog={auditLog} />
